...
export const USE_SERVICE_MOCK = process.env.NEXT_PUBLIC_USE_MOCK !== 'false'

// ─── Dashboard ───────────────────────────────────────────────────────────────

// GET /api/v1/dashboard
export async function getDashboardStatus(): Promise<DashboardStatus> {
  if (!USE_SERVICE_MOCK) return apiFetch<DashboardStatus>('/api/v1/dashboard')
  return MOCK_DASHBOARD_STATUS
}

// ─── Module 1: Documents ─────────────────────────────────────────────────────

// GET /api/v1/documents (session cookie auth)
...
export async function getAiExtractions(): Promise<AiExtraction[]> {
  if (!USE_SERVICE_MOCK)
    return apiFetch<AiExtraction[]>('/api/v1/documents/extractions')
  return MOCK_AI_EXTRACTIONS
}

/** Register a document already uploaded to Firebase Storage (preferred flow). */
export async function registerUploadedDocument(payload: {
  documentId: string
  storagePath: string
  originalFilename: string
  sizeKb: number
}): Promise<UploadedDocument> {
  return apiFetch<UploadedDocument>('/api/v1/documents/register', {
    method: 'POST',
...
export async function getProfile(): Promise<TaxProfile> {
  if (!USE_SERVICE_MOCK) return apiFetch<TaxProfile>('/api/v1/profile')
...
export async function getReliefs(): Promise<Relief[]> {
  if (!USE_SERVICE_MOCK) return apiFetch<Relief[]>('/api/v1/reliefs')
  return MOCK_RELIEFS
}
...
export async function getTaxSummary(): Promise<TaxSummary> {
  if (!USE_SERVICE_MOCK) return apiFetch<TaxSummary>('/api/v1/summary')
...
export async function getFilingData(): Promise<FilingData> {
  if (!USE_SERVICE_MOCK) return apiFetch<FilingData>('/api/v1/filing')
...
export async function getChatHistory(): Promise<ChatMessage[]> {
  if (!USE_SERVICE_MOCK) return apiFetch<ChatMessage[]>('/api/v1/chat/history')
  return MOCK_CHAT_MESSAGES
}

// GET /api/v1/chat/context
export async function getChatContext(): Promise<ChatContext> {
  if (!USE_SERVICE_MOCK) return apiFetch<ChatContext>('/api/v1/chat/context')
